    ...
    return (
      <aside className={styles.sideNav}>
        ...
        <div className={styles.databases}>
          ...
          {selectedDb ? <a href=""><svg><use xlinkHref="#icon-settings" /></svg></a> : null}
        </div>

        <div className={styles.collections}>
          <h3 className={styles.h3}>Collections</h3>
          <a href="" onClick={this.onAddCollectionClick}><svg><use xlinkHref="#icon-add" /></svg></a>
          <nav className={styles.nav}>
            {
              collections.map((collection, i) =>
                <a href="" key={`col_${i}`} className={collection == selectedCollection ? styles.collectionActive : styles.collection} onClick={this.onCollectionSelect(collection)}>
                  <svg><use xlinkHref="#icon-apps" /></svg>{collection}
                </a>
              )
            }
          </nav>
