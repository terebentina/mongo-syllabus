  ...
  return (dispatch, getState) => {
    ...
    if (pageNum != state.currentPage) {
      return Promise.all([
        ...
        request.get(`${Constants.API_URL}/api/docs/${state.selectedDb}/${state.selectedCollection}`, {
          query: state.filter.query,
          p: pageNum,
          limit: state.filter.limit,
        }).then(
          (json) => dispatch(receiveDocs(state.selectedCollection, pageNum, json))
        ).catch((err) => Promise.all([
...
function removeDoc(db, collection, docId) {
  return (dispatch) => request.delete(`${Constants.API_URL}/api/docs/${db}/${collection}/${docId}`)
...
/**
 * this dispatches the confirmation which is handled by a component. If the user confirms, the
 * component will call the function provided by this action which, in turn, dispatches remove
 * @param db
 * @param collection
 * @param docId
 * @returns {Function}
...
export function updateDocument(oldData, doc) {
  return (dispatch) => request.put(`${Constants.API_URL}/api/docs/${oldData.db}/${oldData.collection}/${oldData.doc._id}`, { doc })
...
export function createCollection(db, collectionName) {
  return (dispatch) => request.post(`${Constants.API_URL}/api/collections/${db}/${collectionName}`)
    .then(() => Promise.all([
...
export function renameCollection(oldData, newName) {
  return (dispatch) => request.put(`${Constants.API_URL}/api/collections/${oldData.db}/${oldData.collection}`, { collection: newName })
    .then(() => Promise.all([
...
function dropCollection(db, collection) {
  return (dispatch) => request.delete(`${Constants.API_URL}/api/collections/${db}/${collection}`)
    .then(() => Promise.all([
...
export function setViewMode(mode) {
  return {
    type: Constants.SET_VIEW_MODE, mode,
  };
}
